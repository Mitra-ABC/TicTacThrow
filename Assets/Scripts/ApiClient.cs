...
public class ApiClient : MonoBehaviour
{
    ...
    public IEnumerator VerifyIAP(string sku, string token, string store, Action<VerifyIAPResponse> onSuccess, Action<string> onError)
    {
        ...
        Debug.Log($"[IAP] ApiClient.VerifyIAP: sending request, url={url}, sku={sku}, store={store}");
        ...
        using (var request = new UnityWebRequest(url, "POST"))
        {
            ...
            {
                var errorMessage = ExtractErrorMessage(request);
                Debug.Log($"[IAP] ApiClient.VerifyIAP: error — {errorMessage}");
            ...
            var responseText = request.downloadHandler.text;
            Debug.Log($"[IAP] ApiClient.VerifyIAP: server response — {responseText}");
            Log($"[ApiClient] VerifyIAP response: {responseText}");
            try
            {
                var data = ApiResponseParser.ParseVerifyIAPResponse(responseText);
                Debug.Log($"[IAP] ApiClient.VerifyIAP: parse ok, status={data?.status}, message={data?.message}");
                onSuccess?.Invoke(data);
            }
            catch (Exception ex)
            {
                Debug.Log($"[IAP] ApiClient.VerifyIAP: parse error — {ex.Message}");
